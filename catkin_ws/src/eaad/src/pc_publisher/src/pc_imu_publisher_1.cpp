...
void Publish_IMU ( ros::Publisher, std::vector < float >, std::string );
...
main (int argc, char** argv)
{
  ...
  if ( LOG_RUN_TIME )
  {
    ...
    imu_stamp.open(imu_stamp_path,std::ifstream::in);
  ...
  //{
    ...
    for (const std::string& filename1 : files_in_directory) {
      ...
      std::string line_imu="";
      std::getline(imu_stamp, line_imu);
      std::string line_vehicle="";
      std::getline(vehicle_stamp, line_vehicle);
      Parse_CSV_Line ( csv_line, parsed_csv_line );
      Publish_IMU ( imu_pub, parsed_csv_line,line_imu);
...
// Publish IMU
void Publish_IMU ( ros::Publisher pub, std::vector < float > imu_values, std::string line)
{
  ...
  imu_msg.header.stamp = ros::Time(std::stod(line));
  ...
  imu_msg.angular_velocity.x = -imu_values.at ( GYROSCOPE_X );
  imu_msg.angular_velocity.y = imu_values.at ( GYROSCOPE_Y );
  imu_msg.angular_velocity.z = -imu_values.at ( GYROSCOPE_Z );

  // Linear acceleration
  imu_msg.linear_acceleration.x = imu_values.at ( ACCELEROMETER_X );
  imu_msg.linear_acceleration.y = -imu_values.at ( ACCELEROMETER_Y );
  imu_msg.linear_acceleration.z = imu_values.at ( ACCELEROMETER_Z );
...
//   }
  ...
  if ( n_h.getParam ("/imu_timestamp_path", imu_stamp_path ) )
      std::cout << "IMU Timestamp path: " << imu_stamp_path << std::endl;
  else
  {
      std::cout << "IMU Timestamp path not set" << std::endl;
      return -1;
  }
...
bool Read_Point_Cloud 
  ( const std::string path_to_file, pcl::PCLPointCloud2::Ptr cloud, std:: string & filename)
{
...
void Publish_Cloud ( int lidar_id, int frame_number, pcl::PCLPointCloud2::Ptr input_cloud, std:: string filename, std:: string line_vehicle)
{
  ...
  ros::Time timestamp;
  timestamp=ros::Time(std::stod(line_vehicle));
