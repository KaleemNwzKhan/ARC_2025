...
pcl::PointCloud<pcl::PointXYZ>::Ptr NDT_Align(pcl::Registration<pcl::PointXYZ, pcl::PointXYZ>::Ptr registration, const pcl::PointCloud<pcl::PointXYZ>::Ptr &target_cloud, const pcl::PointCloud<pcl::PointXYZ>::Ptr &source_cloud);
...
int main(int argc, char **argv) {
 struct
  ...
  if (pcl::io::loadPCDFile<pcl::PointXYZ> (path_to_cloud/filename1, *point_cloud) == -1)
  {
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr downsampled_vehicle(new pcl::PointCloud<pcl::PointXYZ>());
    pcl::VoxelGrid<pcl::PointXYZ> voxelgrid;
    voxelgrid.setLeafSize(0.8f, 0.8f, 0.8f);
    voxelgrid.setInputCloud(map_cloud);
    voxelgrid.filter(*downsampled_vehicle);

    pcl::PointCloud<pcl::PointXYZ>::Ptr downsampled_infra(new pcl::PointCloud<pcl::PointXYZ>());
    pcl::VoxelGrid<pcl::PointXYZ> voxelgrid1;
    voxelgrid1.setLeafSize(0.6f, 0.6f, 0.6f);
    voxelgrid1.setInputCloud(point_cloud);
    voxelgrid1.filter(*downsampled_infra);
    ...
    aligned = NDT_Align(ndt_omp, downsampled_vehicle, downsampled_infra);
  }
   while (not q.empty ())
    {
