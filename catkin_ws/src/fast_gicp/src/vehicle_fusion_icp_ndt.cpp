...
ros::Subscriber vehicle_pc_sub, vehicle_pc_sub_2, diff_compress_sub, diff_pc_sub, vehicle_transform_sub;
...
Eigen::Matrix4f end_to_end_transform;
...
void callback_icp(const sensor_msgs::PointCloud2ConstPtr& msg, const fast_gicp::TransformConstPtr& vehicle_transform);
...
int main(int argc, char **argv) {
  ...
  typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::PointCloud2, fast_gicp::Transform> MySyncPolicy_icp;
  ...
  typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::PointCloud2, fast_gicp::CompressDiff> MySyncPolicy;
  message_filters::Synchronizer<MySyncPolicy> sync(MySyncPolicy(1),vehicle_pc_sub, diff_compress_sub);
  sync.registerCallback(boost::bind(&callback, _1, _2));

  //message_filters::Synchronizer<MySyncPolicy_icp> sync_icp(MySyncPolicy_icp(1),vehicle_pc_sub);

  //sync_icp.registerCallback(boost::bind(&callback_icp, _1, _2));


  vehicle_pc_sub_2 = nh.subscribe( vehicle_pc_topic, QUEUE_SIZE_SUB,callback_icp);
...
//     pcl::transformPointCloud(*infra_cloud, *infra_cloud, end_to_end_transform);
...
//     // auto duration_transform_infra = (t6 - t5).toSec();
//     // auto duration_fusion = (t7 - t6).toSec();
//     // auto duration_total = (t7 - t1).toSec();
...
//     // std::cout << "duration_total :" << duration_total * 1000 << ": msec" << std::endl;
    ...
    std::cout << "start_time_callback:" << t1.toSec() * 1000 << ": msec" << std::endl;
...
void callback_icp(const sensor_msgs::PointCloud2ConstPtr& msg, const fast_gicp::TransformConstPtr& vehicle_transform){
    ...
    // end_to_end_transform = vgicp_cuda.getFinalTransformation();
    std::cout << "start_time_icp :" << t1.toSec() * 1000 << ": msec" << std::endl;
