...
int main(int argc, char **argv) {
  ...
  if (append ==1){
    log_time_ndt.open("/workspace/log_time_ndt.csv",std::ios::app );
    ndt_poses.open("/workspace/ndt.txt", std::ios::app );
  }
  else {
    log_time_ndt.open("/workspace/log_time_ndt.csv" );
    ndt_poses.open("/workspace/ndt.txt" );
    log_time_ndt << "frame_number" << "," << "fitness" << "," << "latency_downsample" << "," << "latency_ndt" << ","<< "latency_total"<<std::endl;
  }
...
void Point_Cloud_Callback(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> msg) {
  ...
  log_time_ndt << duration_downsample << "," << duration_ndt << "," << duration_total << std::endl;
