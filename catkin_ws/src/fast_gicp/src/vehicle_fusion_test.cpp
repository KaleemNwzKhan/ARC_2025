...
void callback_vehicle(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> msg) 
{
  ...
  *aligned +=  *vehicle_cloud ;
  ...
  auto duration_icp_initial_guess= (t2 - t1).toSec() * 1000;
  auto duration_icp= (t3 - t2).toSec()* 1000;
  auto duration_transform= (t4 - t3).toSec() * 1000;
  auto duration_fusion= (t5 - t4).toSec() * 1000;

  log_file_vehicle << pc_counter << ","<< duration_icp_initial_guess << ","<< duration_icp << "," << fitness_score << ","<< duration_transform<< ","
  << duration_fusion << std::endl;
