...
std::string vehicle_2_updated_pc_topic="";
...
ros::Publisher vehicle_pc_pub;


// global variables
const std::string NODE_NAME = "vehicle_ndt_2";
...
int main(int argc, char **argv) {
  ...
  if (append ==1){
    log_time_ndt.open("/workspace/log_time_ndt_v2.csv",std::ios::app );
    ndt_poses.open("/workspace/ndt_v2.txt", std::ios::app );
  }
  else {
    log_time_ndt.open("/workspace/log_time_ndt_v2.csv" );
    ndt_poses.open("/workspace/ndt_2.txt" );
    log_time_ndt << "frame_number" << "," << "latency_downsample" << "," << "fitness" << "," << "latency_ndt"<<std::endl;
  }
  ...
  Advertise_String_Topic(&nh, vehicle_pc_topic, &vehicle_pc_pub, QUEUE_SIZE_PUB);

  // Advertise_PC_Topic(&nh,vehicle_pc_topic,&vehicle_pc_pub, QUEUE_SIZE_PUB);
  
  init_guess = Initialize_Init_Guess(vehicle_initial_guess_path);

  std::cout << "Subscribing to point cloud topic: " << vehicle_2_updated_pc_topic << std::endl;
  pc_sub = nh.subscribe(vehicle_2_updated_pc_topic, QUEUE_SIZE_SUB, Point_Cloud_Callback);
...
int Sanity_Check(ros::NodeHandle n_h) {
  ...
  if(n_h.getParam("/vehicle_2_pc_topic", vehicle_pc_topic))
  ...
  if(n_h.getParam("/vehicle_2_transform_topic", vehicle_transform_topic))
  ...
  if(n_h.getParam("/vehicle_2_initial_guess_path", vehicle_initial_guess_path))
    ...
    if(n_h.getParam("/updated_v2_pc_topic", vehicle_2_updated_pc_topic))
    std::cout << "updated_v2_pc_topic: " << vehicle_2_updated_pc_topic << std::endl;
  else {
    std::cout << "/vehicle_pc_topic parameter not set" << std::endl;
    return -1;
  }
...
void Point_Cloud_Callback(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> msg) {
  ...
  octree_compression.encodePointCloud(raw_input_cloud, compressedData);
  compressed_diff.data = compressedData.str();
  Publish_String(&vehicle_pc_pub, pc_counter++, compressed_diff);
