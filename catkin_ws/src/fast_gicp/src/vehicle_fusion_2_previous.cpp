...
Eigen::Matrix4f vehicle_pose_inv, vehicle_pose_previous, vehicle_pose_current;
...
int main(int argc, char **argv) {
  ...
  //log_file_vehicle_transform << "frame_number" << "," << "icp_initial_guess" <<","<< "icp" <<"," <<"fitness" <<","<<"transformation"  << ","<< "fusion"<<std::endl;
...
void callback_transform( const fast_gicp::TransformConstPtr& vehicle_transform) 
{
  ...
  // find initial guess
  //vehicle_pose_inv =  Get_Transform_as_matrix(vehicle_transform).inverse();
  vehicle_pose_current = Get_Transform_as_matrix(vehicle_transform);

  vehicle_pose_inv = vehicle_pose_previous.inverse();
  ...
  *aligned +=  *vehicle_cloud ;
  ...
  log_file_vehicle_transform << vehicle_transform->header.frame_id  << ","<< duration_icp_initial_guess << ","<< duration_icp << "," << fitness_score << ","<< duration_transform<< ","
  << duration_fusion << std::endl;

  //log_synchoronization << before_icp_vehicle_pcd_frame<< "," << vehicle_pcd_frame << "," << vehicle_transform->header.frame_id << std::endl; 
  //pc_counter++;
  //std::cout << "Latency :" << duration * 1000 << ": msec" << std::endl;

  //*vehicle_cloud += *infra_cloud;
  Publish_Cloud(&fused_pc_pub, pc_counter++, aligned);
  //Publish_Cloud(&fused_pc_pub1, pc_counter, aligned1);
