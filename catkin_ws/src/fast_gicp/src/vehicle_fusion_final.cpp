...
Eigen::Matrix4f vehicle_pose_inv;
fast_gicp::Transform tf;


// global objects
std::mutex m_reconstructed, m_vehicle;
...
int main(int argc, char **argv) {
  ...
  // log_file_vehicle.open("/workspace/log_file_vehicle_pcd.csv",std::ios::app);
  // log_file_infra.open("/workspace/log_file_infra.csv",std::ios::app);
  // log_file_vehicle_transform.open("/workspace/log_file_vehicle_transform.csv",std::ios::app);
  // EtE_with_icp.open("/workspace/EtE_with_icp.txt", std::ios::app);
  // EtE_without_icp.open("/workspace/EtE_without_icp.txt", std::ios::app);


  log_file_vehicle.open("/workspace/log_file_vehicle_pcd.csv");
  log_file_infra.open("/workspace/log_file_infra.csv");
  log_file_vehicle_transform.open("/workspace/log_file_vehicle_transform.csv");
  EtE_with_icp.open("/workspace/EtE_with_icp.txt");
  EtE_without_icp.open("/workspace/EtE_without_icp.txt");


  // log_file_vehicle << "frame_number" << "," << "latency_downsample_vehicle" << "," << "icp_initial_guess" <<","<< "icp" <<"," <<"fitness" <<","<<"transformation"  << ","<< "fusion"<<  "," << "total_icp" << std::endl;
  // log_file_infra << "frame_number" << ","  << "latency_network"<< "," << "latency_decompression" << "," <<"latency_reconstruction" << "," << " latency_total" <<std::endl;
  // log_file_vehicle_transform << "frame_number" << ","<< "vehicle_transform"<<std::endl;
  ...
  vehicle_pose_current= Initialize_Init_Guess(vehicle_initial_guess_path); //.inverse();
  vehicle_pose_previous = vehicle_pose_current;
  ...
  //*d_infra_static_pc = *infra_static_pc;
...
void callback_vehicle(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> msg) 
{


  auto t1 = ros::WallTime::now();

   // make vgicp object. NOTE: making a global vgicp object dosent give expected results
  fast_gicp::FastVGICPCuda<pcl::PointXYZ, pcl::PointXYZ> vgicp_cuda;
  vgicp_cuda.setResolution(1);
  vgicp_cuda.setMaximumIterations(100);
  vgicp_cuda.setNearestNeighborSearchMethod(fast_gicp::NearestNeighborMethod::GPU_RBF_KERNEL);
  ...
  Eigen::Matrix4f transient = vehicle_pose_current - vehicle_pose_previous;

  Eigen::Matrix4f vehicle_pose_interpolated = vehicle_pose_current;
  vehicle_pose_interpolated(0,3) += transient(0,3);
  vehicle_pose_interpolated(1,3) += transient(1,3);
  vehicle_pose_interpolated(2,3) += transient(2,3);
  vehicle_pose_inv = vehicle_pose_interpolated.inverse();
  ...
  *aligned +=  *vehicle_cloud ;
  ...
  auto duration_icp_initial_guess= (t3 - t2).toSec() * 1000;
  auto duration_icp= (t4 - t3).toSec()* 1000;
  auto duration_transform= (t5 - t4).toSec() * 1000;
  auto duration_fusion= (t6 - t5).toSec() * 1000;
  auto duration_total= (t6 - t1).toSec() * 1000;

  log_file_vehicle << pc_counter << ","<< duration_downsample_vehicle << "," << duration_icp_initial_guess << ","<< duration_icp << "," << fitness_score << ","<< duration_transform<< ","
  << duration_fusion << "," << duration_total <<std::endl;
...
void callback_transform( const fast_gicp::TransformConstPtr& vehicle_transform) 
{
  ...
  // find initial guess
  vehicle_pose_current =  Get_Transform_as_matrix(vehicle_transform); //.inverse();
  ...
  log_file_vehicle_transform << vehicle_transform->header.frame_id  << ","<< duration_vehicle_transform << std::endl;
...
void callback_lidar( const fast_gicp::CompressDiffConstPtr& diff_compressed) 
{
  ...
  *reconstructed = *infra_static_pc + *infra_diff_cloud;
  ...
  auto duration_decompress= (t2 - t1).toSec() * 1000; 
  auto duration_reconstruction = (t3 - t2).toSec() * 1000; 
  ...
  log_file_infra << diff_compressed->header.frame_id << "," << duration_network << ","<< duration_decompress << "," <<  duration_reconstruction << "," << duration_total <<std::endl;
