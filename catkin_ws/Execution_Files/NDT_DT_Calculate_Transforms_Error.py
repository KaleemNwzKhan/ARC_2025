...
c = sys.argv[3]  # File containing number of cells
f = open(sys.argv[4], "a")  # Output file for RTE
g = open(sys.argv[5], "a")  # Output file for RRE
...
# Read cell count values
Cell_Values = list(map(int, open(c, 'r').read().splitlines()))
...
for k in range(len(G_Pose)):
    if Cell_Values[k] < 100:
        continue  # Skip iteration if cell count is less than 100
...
if valid_count > 0:
    ...
    print("RTE:", RTE / valid_count)
    print("RRE:", RRE / valid_count)
else:
    print("No valid entries to compute RTE/RRE.")
