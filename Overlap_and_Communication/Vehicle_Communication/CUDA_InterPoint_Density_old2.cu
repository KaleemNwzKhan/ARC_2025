...
std::string dataset_folder, Map, Vehicle_PCDs_Path, Leader_PCD_Path;
float accumulate_time=0, time1, time2,time3,time3_1,time4, time5, time6, time7, time8,time9,time10, run_time_start,run_time_end,run_time_start1,run_time_end1;
...
// ******************Grid Estimation for the Intersection***********************  
    ...
    float time4 = (((float)(run_time_end - run_time_start) / CLOCKS_PER_SEC) * 1000);
...
// ******************Vehicle Drivable Space Estimation***********************  
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ> (Vehicle_path, *Vehicle_cloud_temp) == -1) //* load the file
    {
        PCL_ERROR ("Couldn't read file  \n");
        return (-1);
    }
...
// ***************************************** Dynamic Objects and Non-Road Points Extraction**********************************************  
    ...
    time3_1 = time3;
        ...
        // pcl::io::savePCDFileASCII("Vehicles_Points_Only.pcd", *changes);
    ...
    // // pcl::io::savePCDFileASCII("Non_Ground_Points.pcd", *Non_ground_points);
...
// // *****************************************BlindSpots Assignment to Suppliers*********************************************************************   
    ...
    // pcl::io::savePCDFileASCII("bounding_box_centers.pcd", *centers_cloud);
...
// // *****************************************Overlap Estimation*********************************************************************   
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ> (Leader_PCD_Path, *Leader_cloud) == -1) //* load the file
    {
        PCL_ERROR ("Couldn't read file  \n");
        return (-1);
    }
...

    {
        ...
        cudaDeviceSynchronize();
        run_time_end = clock();
        time9=time9+(run_time_end-run_time_start);
        cudaMemcpy(res_ptr, d_res_ptr, cloudSize * sizeof(int), cudaMemcpyDeviceToHost);
        cudaFree(d_points);
        cudaFree(d_res_ptr);
    ...
    numBlocks = (Non_ground_points->points.size() + blockSize - 1) / blockSize; // Adjust for the number of points in the vehicle cloud
    run_time_start = clock();
    populateFinalCloud<<<numBlocks, blockSize>>>(d_vehicle_points,d_leader_points,d_res_ptr,d_res_ptr_leader, d_indices_above_threshold, above_threshold, d_final_vehicle_points,d_final_leader_points, d_final_count_vehicle,d_final_count_leader,Non_ground_points->points.size());
    cudaDeviceSynchronize();
    run_time_end = clock();
    time9=time9+(run_time_end-run_time_start);   
    cudaCheckError();
    ...
    // pcl::io::savePCDFileASCII("Vehicle_Overlap.pcd", *final_cloud_Vehicle);
    ...
    // pcl::io::savePCDFileASCII("Leader_Overlap.pcd", *final_cloud_Leader);
