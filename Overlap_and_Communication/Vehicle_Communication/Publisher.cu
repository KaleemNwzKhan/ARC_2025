...
const std::string DIR_OUTPUT = "output_directory";   // Directory for writing matched results
const std::string BASE_DIR = "output_directory";   // Directory for writing matched results
...
float getPointCloudSizeInKB(const pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
    // Each point in pcl::PointXYZ contains three floats (x, y, z)
    const size_t point_size = 3 * sizeof(float); // 3 floats, each 4 bytes
    size_t cloud_size_bytes = cloud->points.size() * point_size; // Total size in bytes
    
    // Convert to kilobytes (1 KB = 1024 bytes)
    float cloud_size_kb = cloud_size_bytes / 1024.0f;
    return cloud_size_kb;
}
...
// Step 3: Store only centroids that have at least one assigned point
std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> non_empty_centroid_clouds;
pcl::PointCloud<pcl::PointXYZ>::Ptr assigned_centroids(new pcl::PointCloud<pcl::PointXYZ>());

for (size_t i = 0; i < centroids.size(); ++i) {
    if (point_counts[i] > 0) {  
        assigned_centroids->points.push_back(centroids[i]);  // Store the centroid position
        non_empty_centroid_clouds.push_back(centroid_clouds[i]);  // Store its corresponding points
    }
}
...
float cloud_size_kb=0;

char c;
pcl::PointCloud<pcl::PointXYZ>::Ptr frame_point_cloud(new pcl::PointCloud<pcl::PointXYZ>());
std::vector<int> requested_cells; 


std::string current_vehicle_folder = "Vehicle_" + std::to_string(Vehicle_number);
std::string frame_file = std::to_string(global_count+1) + ".txt";
std::string file_path = BASE_DIR + "/" + current_vehicle_folder + "/" + frame_file;
...
{ 
    float size = getPointCloudSizeInKB(frame_point_cloud);
    cloud_size_kb += size;
...
BB_size_file<<cloud_size_kb<<endl;
totaltime = (((float)(totaltime)/CLOCKS_PER_SEC)*1000);
...
//                 // Check if the cell is already in the requested_cells vector
//                 if (std::find(requested_cells.begin(), requested_cells.end(), cell) == requested_cells.end()) {
...
int main (int argc, char** argv)
{
   ...
   // V_map_file.open(V_Map_transform_file);
    ...
    log_file<<"Producer_Latency"<<endl;
