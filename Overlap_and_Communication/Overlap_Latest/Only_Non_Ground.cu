...
std::vector<std::vector<Eigen::Matrix4f>> World_Transforms(1, std::vector<Eigen::Matrix4f>(number_of_frames));
...
// ******************Vehicle Drivable Space Estimation***********************  
    ...
    std::vector<std::filesystem::path> files_in_directory1;
    std::copy(std::filesystem::directory_iterator(Leader_PCD_Path), std::filesystem::directory_iterator(), std::back_inserter(files_in_directory1));
...
for (const std::string& filename : files_in_directory1)
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr Leader_cloud (new pcl::PointCloud<pcl::PointXYZ>);
        pcl::PointCloud<pcl::PointXYZ>::Ptr Leader_cloud_temp (new pcl::PointCloud<pcl::PointXYZ>);
    ...
    if (pcl::io::loadPCDFile<pcl::PointXYZ> (Leader_PCD_Path+"/" +current_frame_name, *Leader_cloud) == -1) //* load the file
    {
        ...
        if (pcl::io::loadPCDFile<pcl::PointXYZ> (Leader_PCD_Path+"/" +current_frame_name, *Leader_cloud_temp) == -1) //* load the file
    {
        PCL_ERROR ("Couldn't read file  \n");
        return (-1);
    }
    ...
    y_pass_y.setInputCloud(Leader_cloud);
    ...
    Leader_cloud = xy_cloud_filtered_z;
...
// ***************************************** Dynamic Objects and Non-Road Points Extraction**********************************************  
    ...
    detectChangesCUDA(cloud_downsampled_map, Leader_cloud, changes, distance_threshold);
    time3_1 = time3;
    ...
    detectChangesCUDA(cloud_downsampled_vehicle, Leader_cloud_temp, Non_ground_points, distance_threshold);
    ...
    pcl::transformPointCloud(*Non_ground_points, *Non_ground_points, leader_trans_inverse);
    pcl::io::savePCDFileBinary("Dataset/Leader_Non_Ground/"+current_frame_name, *Non_ground_points);
