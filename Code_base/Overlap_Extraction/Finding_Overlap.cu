...
std::ofstream log_file,BB_file,cells_count_file;
std::ifstream vehicle_file,leader_file,Transform_file;
Eigen::Matrix4f Vehicle_transform,vehicle_trans;
int ipd_threshold = 10;


std::string dataset_folder, Map, Vehicle_PCDs_Path, Leader_PCD_Path,Leader_transform_file, Vehicle_transform_file, Overlap_Folder, Output_file;
...
// // *****************************************Overlap Estimation*********************************************************************   
    ...
    pcl::getMinMax3D (*Vehicle_cloud_temp, minPt_vehicle, maxPt_vehicle);
    ...
    {
        int cloudSize_v = Vehicle_cloud_temp->size();
        result2.resize(cloudSize_v);

        pcl::PointXYZ *h_points =Vehicle_cloud_temp->points.data();  // Get pointer to the points array
        pcl::PointXYZ *d_points;
    ...
    cudaMalloc((void**)&d_res_ptr,Vehicle_cloud_temp->points.size() * sizeof(int));
    cudaMemcpy(d_res_ptr, res_ptr, Vehicle_cloud_temp->points.size() * sizeof(int), cudaMemcpyHostToDevice);
    ...
    cudaMalloc(&d_vehicle_points, Vehicle_cloud_temp->points.size() * sizeof(pcl::PointXYZ));
    cudaMalloc(&d_leader_points, Leader_cloud->points.size() * sizeof(pcl::PointXYZ));
    cudaMalloc(&d_final_vehicle_points, Vehicle_cloud_temp->points.size() * sizeof(pcl::PointXYZ)); // Allocate for worst case
    ...
    cudaMemcpy(d_vehicle_points, Vehicle_cloud_temp->points.data(), Vehicle_cloud_temp->points.size() * sizeof(pcl::PointXYZ), cudaMemcpyHostToDevice);
    ...
    cells_count_file<<above_threshold<<endl;
    if (above_threshold<10)
	{overlap=false;}
    // Launch kernel to populate final_cloud based on the result2 mapping and threshold indices
    numBlocks = (Vehicle_cloud_temp->points.size() + blockSize - 1) / blockSize; // Adjust for the number of points in the vehicle cloud
    run_time_start = clock();
    populateFinalCloud<<<numBlocks, blockSize>>>(d_vehicle_points,d_leader_points,d_res_ptr,d_res_ptr_leader, d_indices_above_threshold, above_threshold, d_final_vehicle_points,d_final_leader_points, d_final_count_vehicle,d_final_count_leader,Vehicle_cloud_temp->points.size());
    cudaDeviceSynchronize();
    run_time_end = clock();
    time_overlap=time_overlap+(run_time_end-run_time_start);   
    cudaCheckError();
...
if (overlap)
{
    ...
    pcl::transformPointCloud(*final_cloud_Vehicle, *final_cloud_Vehicle, vehicle_trans_inverse);
     ...
     pcl::io::savePCDFileASCII(Overlap_Folder+"Vehicle/"+current_frame_name, *final_cloud_Vehicle);
    ...
    pcl::transformPointCloud(*final_cloud_Leader, *final_cloud_Leader, leader_trans_inverse);
     ...
     pcl::io::savePCDFileASCII(Overlap_Folder+"Leader/"+current_frame_name, *final_cloud_Leader);
}
...

{
    ...
    Output_file= dataset_folder +"/"+Output_file+".txt";
