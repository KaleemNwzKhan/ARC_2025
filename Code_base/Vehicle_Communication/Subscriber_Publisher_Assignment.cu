...
const std::string DIR_SOURCE = "/dataset/Communication/BlindSpots";   // Directory containing files to process
const std::string DIR_COMPARE = "/dataset/Communication/Vehicles_Cells"; // Directory containing files for comparison
const std::string DIR_OUTPUT = "/dataset/Communication/Required";   // Directory for writing matched results
...
for (int v = 0; v < NUM_VEHICLES; ++v) {
    ...
    std::string latencies_file = "/dataset/Communication/Producer_Assignment_Latencies/" + std::to_string(v) + ".txt";
    ...
    for (int frame = 1; frame <= NUM_FRAMES; ++frame) {
        ...
        std::unordered_set<float> blindspotCells = readValuesFromFile(vehicleFilePath);
        if (blindspotCells.empty()) continue;

        std::unordered_map<float, std::pair<int, std::string>> best_matches; // cell_id → (max_points, vehicle_dir)
        double total_compute_time = 0.0;

        std::unordered_set<float> already_assigned;
        ...
        for (int v_compare = 0; v_compare < NUM_VEHICLES; ++v_compare) {
            ...
            std::ifstream compareFile(compareFilePath);
            if (!compareFile.is_open()) continue;

            std::unordered_map<float, int> compareMap;
            float cell_id;
            int num_points;

            while (compareFile >> cell_id >> num_points) {
                compareMap[cell_id] = num_points;
            }

            auto start_time = std::chrono::high_resolution_clock::now();

            for (const float& cell : blindspotCells) {
                if (already_assigned.count(cell)) continue;

                auto it = compareMap.find(cell);
                if (it != compareMap.end()) {
                    int points = it->second;
                    auto& entry = best_matches[cell];
                    if (entry.first < points) {
                        entry = {points, compareDir};
                        if (points >= 20) {
                            already_assigned.insert(cell); // Early stop for this cell
                        }
                    }
                }
            }
            ...
            total_compute_time += std::chrono::duration<double, std::milli>(end_time - start_time).count();
        }

        // Finalize matches and remove assigned cells from blindspot
        std::vector<std::pair<float, std::string>> matches;
        std::unordered_set<float> assignedCells;

        for (const auto& [cell_id, info] : best_matches) {
            matches.emplace_back(cell_id, info.second);
            assignedCells.insert(cell_id);
        }

        for (const float& id : assignedCells) {
            blindspotCells.erase(id);
        }

        file1 << total_compute_time << "\n";

        std::string outputDir = DIR_OUTPUT + "/" + vehicleDir;
        fs::create_directories(outputDir);

        writeMatchesToFile(outputDir + "/" + frameFile, matches);
        writeRemainingValuesToFile(vehicleFilePath, blindspotCells);
    }
