...
// Cell size
float cell = 2.5f;
float half_center = (7 * cell) / 2.0f;
float x_min_center = xC - half_center;
float x_max_center = xC + half_center;
float y_min_center = yC - half_center;
float y_max_center = yC + half_center;

// Create groups
pcl::PointCloud<pcl::PointXYZ>::Ptr group_A(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_B(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_C(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_D(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_E(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_F(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_G(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_H(new pcl::PointCloud<pcl::PointXYZ>());
pcl::PointCloud<pcl::PointXYZ>::Ptr group_I(new pcl::PointCloud<pcl::PointXYZ>());
...
for (const auto& pt : centroids) {
    ...
    // Center
    if (x >= x_min_center && x <= x_max_center &&
        y >= y_min_center && y <= y_max_center) {
        group_E->points.push_back(pt);
        continue;
    }

    // LEFT road: x < x_min_center
    if (x < x_min_center && y >= y_min_center && y <= y_max_center) {
        if (y >= yC)
            group_A->points.push_back(pt); // Top half
        else
            group_B->points.push_back(pt); // Bottom half
        continue;
    }

    // RIGHT road: x > x_max_center
    if (x > x_max_center && y >= y_min_center && y <= y_max_center) {
        if (y >= yC)
            group_F->points.push_back(pt); // Top half
        else
            group_G->points.push_back(pt); // Bottom half
        continue;
    }

    // TOP road: y > y_max_center
    if (y > y_max_center && x >= x_min_center && x <= x_max_center) {
        if (x <= xC)
            group_C->points.push_back(pt); // Left half
        else
            group_D->points.push_back(pt); // Right half
        continue;
    }

    // BOTTOM road: y < y_min_center
    if (y < y_min_center && x >= x_min_center && x <= x_max_center) {
        if (x <= xC)
            group_H->points.push_back(pt); // Left half
        else
            group_I->points.push_back(pt); // Right half
        continue;
    }

    std::cout << "Unassigned centroid at (" << x << ", " << y << ")\n";
}
...
text_filename << "/dataset/Communication/Vehicles_Cells/Vehicle_" << static_cast<int>(Vehicle_number)<< "/" << new_filename << ".txt";
...
for (size_t i = 0; i < centroids.size(); ++i) {
    if (point_counts[i] > 0) {
        assigned_centroids_file << i << " "<< point_counts[i]<< "\n"; 
    }
}
...
// Assume vehicle_trans_inverse and leader_trans are already defined 4x4 Eigen::Matrix4f

// Transform vehicle cloud
// Transform vehicle cloud to leader frame
// Transform vehicle cloud to leader frame
// Transform vehicle cloud once per frame (assuming vehicle_trans_inverse and leader_trans updated each frame)
...
// Compute vehicle position in leader frame
Eigen::Vector4f vehicle_origin(0, 0, 0, 1);
Eigen::Vector4f vehicle_pos_leader = leader_trans * vehicle_origin;

position_x = vehicle_pos_leader.x();
position_y = vehicle_pos_leader.y();

// std::cout << "Vehicle position in leader frame: (" 
//           << position_x << ", " << position_y << ")" << std::endl;

// Prepare region clouds once — transform original groups into transformed_groups only once
static bool first_call = true;
static std::vector<std::pair<std::string, pcl::PointCloud<pcl::PointXYZ>::Ptr>> transformed_regions;

if (first_call) {
    std::vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> groups = {
        group_A, group_B, group_C, group_D, group_E,
        group_F, group_G, group_H, group_I
    };

    transformed_regions.clear();

    for (size_t i = 0; i < groups.size(); ++i) {
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_transformed(new pcl::PointCloud<pcl::PointXYZ>);
        pcl::transformPointCloud(*groups[i], *cloud_transformed, vehicle_trans_inverse);
        pcl::transformPointCloud(*cloud_transformed, *cloud_transformed, leader_trans);
        transformed_regions.push_back({std::string(1, 'A' + i), cloud_transformed});
    }
    first_call = false;
}
...
std::vector<RegionBounds> region_bounds;
const float margin_x = 2.5f;
const float margin_y = 2.5f;

// Compute bounding boxes for each region from transformed clouds
for (const auto& [name, cloud] : transformed_regions) {
    if (cloud->empty()) {
        std::cout << "Region " << name << " is empty." << std::endl;
        continue;
    }
    ...
    region_bounds.push_back({name, xmin, xmax, ymin, ymax});

    // Print bounding box info for debugging
    // std::cout << "Region " << name << " bounding box:" << std::endl;
    // std::cout << "  X: [" << xmin << ", " << xmax << "]" << std::endl;
    // std::cout << "  Y: [" << ymin << ", " << ymax << "]" << std::endl;
}

// Assign vehicle to region based on position
std::string vehicle_region = "Unknown";
for (const auto& rb : region_bounds) {
    if (position_x >= rb.xmin && position_x <= rb.xmax &&
        position_y >= rb.ymin && position_y <= rb.ymax) {
        vehicle_region = rb.name;
        break;
    }
}


std::map<std::string, std::set<std::string>> region_filter_map = {
    {"A", {"B", "C", "I"}},
    {"B", {"A", "C", "D", "F", "G", "H", "I","E"}}, // Keep only E
    {"C", {"A", "B", "D", "F", "G", "H", "I", "E"}}, // Keep only E
    {"D", {"F", "B", "C"}},
    {"F", {"A", "B", "C", "D", "G", "H", "I", "E"}}, // Keep only E
    {"G", {"F", "I", "C"}},
    {"I", {"A", "B", "C", "D", "F", "G", "H", "E"}}, // Keep only E
    {"H", {"I", "F", "B"}}
};

std::set<std::string> ignored_regions;
if (region_filter_map.find(vehicle_region) != region_filter_map.end()) {
    ignored_regions = region_filter_map[vehicle_region];
}


// std::cout << "Frame: " << somthing << " Vehicle is in region: " << vehicle_region << std::endl;
// somthing++;
...

{
    ...
    std::stringstream text_filename;
    text_filename << "/dataset/Communication/BlindSpots/Vehicle_" << static_cast<int>(Vehicle_number) << "/" << new_filename << ".txt";
    std::ofstream zero_centroids_file(text_filename.str());


    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_centroids_cloud(new pcl::PointCloud<pcl::PointXYZ>);
    ...
    if (zero_centroids_mask[i] == 1) {
        ...
        // Determine which region the centroid belongs to
        std::string region_of_centroid = "Unknown";
        for (const auto& rb : region_bounds) {
            if (centroids[i].x >= rb.xmin && centroids[i].x <= rb.xmax &&
                centroids[i].y >= rb.ymin && centroids[i].y <= rb.ymax) {
                region_of_centroid = rb.name;
                break;
            }
        }

        // If region of centroid is in ignored list, skip
        if (ignored_regions.find(region_of_centroid) != ignored_regions.end())
            continue;

        // Otherwise, keep this centroid index
        zero_centroids_file << i << "\n";
        // filtered_centroids_cloud->points.push_back(centroids[i]);
    }
}
        // for (size_t i = 0; i < gridSize; ++i) {
        //     if (zero_centroids_mask[i] == 1) {
        ...
        //     }
        // }
//         filtered_centroids_cloud->width = filtered_centroids_cloud->points.size();
// filtered_centroids_cloud->height = 1;
// filtered_centroids_cloud->is_dense = true;

// pcl::io::savePCDFileBinary("filtered_centroids.pcd", *filtered_centroids_cloud);
...
//*****************************************************************************************************************************************    
log_file<<leader_finding_time<<","<<time1+time2<<","<<time3+time4+time5<<","<<time6+time7+time8<<","<<time9<<","<<0<<","<<accumulate_time<<endl;
