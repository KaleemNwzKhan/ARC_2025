...
do
    ...
    VEHICLE_DIR="/dataset/Vehicles/Vehicle_${Vehicle}"
    VEHICLE_DIR_Overlap="/dataset/Leader_Vehicle_Overlap/Vehicle_${Vehicle}"
    ...
    if [ -d "${VEHICLE_DIR}" ]; then
        ...
        LEADER_POSES_FILE="/dataset/Leader/Leader_Poses.txt"
        LEADER_GT_FILE="/dataset/Leader/Leader_GT.txt"

        if [ -f "${GT_FILE}" ] && [ -f "${POSES_FILE}" ]; then
            # Run the Python script with the input files

            python3 Calculate_Vehicle_to_Leader_GT_Transforms.py "${GT_FILE}" "${POSES_FILE}" \
