...
#endif
int stop_count=0;
std::ofstream vehicle_pose_file, initial_guess_list1;
...
Eigen::Matrix4f init_guess;
std::string Infra_path = "/dataset/test/infra_static.pcd";
//std::string Infra_path = "Infrastructure";
std::string Vehicle_path = "/dataset/test/Vehicle";
...
void test(Registration& reg, const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& target, const pcl::PointCloud<pcl::PointXYZ>::ConstPtr& source) {
  ...
  //std::cout<<"Transformation Matrix : "<<reg.getFinalTransformation()<< std::endl;
  double single = std::chrono::duration_cast<std::chrono::nanoseconds>(t2 - t1).count() / 1e6;
  std::cout<<"Latency : "<<single<< std::endl;
  latency_avg= latency_avg+single;  
  }
  
int main(int argc, char** argv) {
std::cout << "Initial Guess : " << std::endl;
  initial_guess_list.open("/dataset/test/init.txt");
  initial_guess_list1.open("/dataset/test/init1.txt");
  vehicle_pose_file.open("/dataset/test/Alignment_Infra_Vehicle.txt");
  ...
  pcl::io::loadPCDFile<pcl::PointXYZ> (Infra_path, *I_point_cloud);

  //std::cout<<"Infra file: "<<files_in_directory_I[iter]<<std::endl;
...
for (const std::string &filename : files_in_directory_V)
  {
   ...
   { */
    ...
    initial_guess_list1 << init_guess << std::endl;
    //std::cout<<"Initial guess: "<<init_guess<<std::endl;
    pcl::io::loadPCDFile<pcl::PointXYZ> (filename, *V_point_cloud);
    std::cout<<"Vehicle file: "<<filename<<std::endl;
 /*    pcl::io::loadPCDFile<pcl::PointXYZ> (files_in_directory_V[iter], *V_point_cloud);
    std::cout<<"Vehicle file: "<<files_in_directory_V[iter]<<std::endl; */
   ...
   /* pcl::io::loadPCDFile<pcl::PointXYZ> (files_in_directory_I[iter], *I_point_cloud);
   std::cout<<"Infra file: "<<files_in_directory_I[iter]<<std::endl; */
...
/*     pcl::PointCloud<pcl::PointXYZ>::Ptr downsampled_I_point_cloud(new pcl::PointCloud<pcl::PointXYZ>());
 ...
 //{ 
  ...
  //vgicp.setTransformationEpsilon(2e-9);
  ...
  //test(vgicp,downsampled_V_point_cloud, downsampled_I_point_cloud);
  //}
 //stop_count++;
   //}
  ...
  vgicp_cuda.setResolution(0.15);
  //vgicp_cuda.setTransformationEpsilon(2e-9);
 ...
 std::cout<<"Average Latency: "<<latency_avg/301<<std::endl;
