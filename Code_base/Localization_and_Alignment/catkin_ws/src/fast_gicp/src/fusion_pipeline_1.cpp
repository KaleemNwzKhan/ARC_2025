...
std::string vehicle_ndt_transform_path ="";
...
std::ifstream initial_guess_list,ndt_transform;
...
void callback_lidar(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> infra_pc);
...
Eigen::Matrix4f read_ndt_transform();
...
int main(int argc, char **argv) {
  ...
  if (append ==1 ){
    ...
    log_file_infra << "frame_number" << ","  << "latency_network"<< "," << "latency_reconstruction" << "," <<"latency_icp" << "," << " latency_fusion" <<std::endl;
    log_file_vehicle_transform << "frame_number" << "," <<"transformation"  << ","<<std::endl;
  }




  // if dont want to append 
  ...
  ndt_transform.open(vehicle_ndt_transform_path);
...
//   }
  ...
  if(n_h.getParam("/vehicle_ndt_transform_path", vehicle_ndt_transform_path))
    std::cout << "vehicle_ndt_transform_path pc topic: " << vehicle_ndt_transform_path << std::endl;
  else {
    std::cout << "/vehicle_ndt_transform_path parameter not set" << std::endl;
    return -1;
  }
...
void callback_transform( const fast_gicp::TransformConstPtr& vehicle_transform) 
{
  //std::cout << "callback_transform" << std::endl;
...
void callback_lidar(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> infra_pc) {
  ...
  pcl::PointCloud<pcl::PointXYZ>::Ptr d_reconstructed (new pcl::PointCloud<pcl::PointXYZ>());

  
  pcl::PointCloud<pcl::PointXYZ>::Ptr raw_input_cloud = infra_pc->makeShared();
  *reconstructed = *raw_input_cloud;
  ...
  m_vehicle_pose.lock();
  Eigen::Matrix4f vehicle_pose_inv = vehicle_pose_current.inverse();
  m_vehicle_pose.unlock();

  //Eigen::Matrix4f vehicle_pose_inv = read_ndt_transform().inverse();

  Eigen::Matrix4f inital_guess =  vehicle_pose_inv * lidar_pose ;


  voxelgrid_infra.setInputCloud(reconstructed);
  voxelgrid_infra.filter(*d_reconstructed);
  ...
  vgicp_cuda.setInputSource(d_reconstructed);
  ...
  *aligned +=  *vehicle_cloud ;
  ...
  auto frame_time = pcl_conversions::fromPCL(infra_pc->header.stamp);
  ...
  auto duration_fusion= (t4 - t3).toSec() * 1000;

  log_file_infra << infra_pc->header.frame_id  << ","<< duration_network << ","<< duration_reconstruction << "," << duration_icp << "," << duration_fusion << std::endl;
