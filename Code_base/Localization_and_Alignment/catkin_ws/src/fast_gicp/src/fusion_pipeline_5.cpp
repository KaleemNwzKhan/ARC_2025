...
std::ofstream log_file_vehicle, log_file_vehicle_transform, log_file_infra, EtE_with_icp, EtE_without_icp, log_file_distance;
...
float distance (Eigen::Matrix4f vehicle_pose) ;
...
int main(int argc, char **argv) {
  ...
  if (append ==1 ){
    ...
    log_file_distance.open("/workspace/log_file_distance.txt", std::ios::app);
  }
    ...
    log_file_distance.open("/workspace/log_file_distance.txt");
    log_file_vehicle << "frame_number" << "," << "latency_downsample_vehicle" << ","<< "latency_icp "<< "," << "latency_fusion" << std::endl;
    ...
    log_file_distance<< "distance" << std::endl;
...
void callback_vehicle(boost::shared_ptr<pcl::PointCloud<pcl::PointXYZ>> vehicle_pc) 
{
  ...
  Eigen::Matrix4f vehicle_pose_inv = vehicle_pose_interpolated.inverse();
  ...
  *aligned +=  *vehicle_cloud ;
  ...
  log_file_vehicle << vehicle_pc->header.frame_id << "," << duration_downsample_vehicle << ","<< duration_icp << ","<< duration_fusion << std::endl;
  //vgicp_cuda.getFitnessScore() << "," 
  log_file_distance << distance(vehicle_pose_previous) << std::endl;
  EtE_with_icp << vgicp_cuda.getFinalTransformation() <<std::endl;
  EtE_without_icp << inital_guess << std::endl;
  std::cout << distance(vehicle_pose_previous) << std::endl;
...
float distance (Eigen::Matrix4f vehicle_pose) {
  ...
  float d =   std::sqrt (std::pow(vehicle_pose(0,3) - lidar_pose(0,3), 2) + std::pow(vehicle_pose(1,3) - lidar_pose(1,3), 2) + std::pow(vehicle_pose(2,3) - lidar_pose(2,3), 2));
  return d;
